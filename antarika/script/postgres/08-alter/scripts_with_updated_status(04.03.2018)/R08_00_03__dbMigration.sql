...
UPDATE public.FpDevice SET status = CASE WHEN status = 'BM' THEN 'BankMade' WHEN status = 'BA' THEN 'BankApproved' WHEN status = 'A' THEN 'Active' WHEN status = 'I' THEN 'Inactive' WHEN status = 'BR' THEN 'ApplicationRejected' WHEN status = 'IE' THEN 'InEditable' ELSE status END  WHERE   status IN ('BM', 'BA','A', 'I','IE', 'BR' );

-- Enable check constraint for deviceStatus
ALTER TABLE public.FpDevice ADD CONSTRAINT ck_deviceStatus_FpDevice check (deviceStatus = 'Assigned' or deviceStatus = 'Unassigned' or deviceStatus = 'Damaged');

ALTER TABLE public.FpDevice ADD CONSTRAINT ck_status_FpDevice check (status = 'BankMade' or status = 'BankApproved' or status = 'Active' or status = 'Inactive' or status = 'Closed' or status = 'Deleted' or status = 'ApplicationRejected' or status = 'Hold');
-- post validation
...
-- Update status value BM=BankMade, BA=BankApproved, A=Active, I=Inactive,BR=BankRejected, IE=Ineditable
UPDATE public.ClientDevice SET status = CASE WHEN status = 'BM' THEN 'BankMade' WHEN status = 'BA' THEN 'BankApproved' WHEN status = 'A' THEN 'Active' WHEN status = 'I' THEN 'Inactive' WHEN status = 'BR' THEN 'ApplicationRejected'  WHEN status = 'IE' THEN 'InEditable' ELSE status END  WHERE   status IN ('BM', 'BA','A', 'I','IE', 'BR' );

-- Enable check constraint for deviceStatus
ALTER TABLE public.ClientDevice ADD CONSTRAINT ck_deviceStatus_ClientDevice check (deviceStatus = 'Assigned' or deviceStatus = 'Unassigned' or deviceStatus = 'Damaged');

-- Enable check constraint for status
ALTER TABLE public.ClientDevice ADD CONSTRAINT ck_status_ClientDevice check (status = 'BankMade' or status = 'BankApproved' or status = 'Active' or status = 'Inactive' or status = 'Closed' or status = 'Deleted' or status = 'ApplicationRejected' or status = 'Hold');
...
-- Update status value BM=BankMade, BA=BankApproved, A=Active, I=Inactive,BR=BankRejected, IE=Ineditable
UPDATE public.PrinterDevice SET status = CASE WHEN status = 'BM' THEN 'BankMade' WHEN status = 'BA' THEN 'BankApproved' WHEN status = 'A' THEN 'Active' WHEN status = 'I' THEN 'Inactive' WHEN status = 'BR' THEN 'ApplicationRejected' WHEN status = 'IE' THEN 'InEditable' ELSE status END  WHERE   status IN ('BM', 'BA','A', 'I','IE', 'BR' );

-- Enable check constraint for deviceStatus
ALTER TABLE public.PrinterDevice ADD CONSTRAINT ck_deviceStatus_PrinterDevice check (deviceStatus = 'Assigned' or deviceStatus = 'Unassigned' or deviceStatus = 'Damaged');

-- Enable check constraint for status
ALTER TABLE public.PrinterDevice ADD CONSTRAINT ck_status_PrinterDevice check (status = 'BankMade' or status = 'BankApproved' or status = 'Active' or status = 'Inactive' or status = 'Closed' or status = 'Deleted' or status = 'ApplicationRejected' or status = 'Hold');
-- post validation
...
-- Update status value BM=BankMade, BA=BankApproved, A=Active, I=Inactive,BR=BankRejected, IE=Ineditable
UPDATE public.CardDevice SET status = CASE WHEN status = 'BM' THEN 'BankMade' WHEN status = 'BA' THEN 'BankApproved' WHEN status = 'A' THEN 'Active' WHEN status = 'I' THEN 'Inactive' WHEN status = 'BR' THEN 'ApplicationRejected' WHEN status = 'IE' THEN 'InEditable'ELSE status END  WHERE   status IN ('BM', 'BA','A', 'I','IE', 'BR' );

-- Enable check constraint for deviceStatus
ALTER TABLE public.CardDevice ADD CONSTRAINT ck_deviceStatus_CardDevice check (deviceStatus = 'Assigned' or deviceStatus = 'Unassigned' or deviceStatus = 'Damaged');

-- Enable check constraint for status
ALTER TABLE public.CardDevice ADD CONSTRAINT ck_status_CardDevice check (status = 'BankMade' or status = 'BankApproved' or status = 'Active' or status = 'Inactive' or status = 'Closed' or status = 'Deleted' or status = 'ApplicationRejected' or status = 'Hold');
-- post validation
